  ...
  return (
    <Box gap="24px">
      <DashboardHero
        ...
        subtitle="Q2 2026 - Live vendor operations - Welcome back"
        actions={
          <Button
            primary
            color="#01A982"
        ...
        metrics={[
          ...
          { accent: '#7C6CF2', title: 'CANDIDATES IN POOL', value: loading ? '...' : candidatesInPool, helper: `${weeklySubmissions.reduce((sum, item) => sum + Number(item.count), 0)} this week`, helperColor: '#01A982' },
          { accent: '#F59E0B', title: 'AVG. TIME TO FILL', value: loading ? '...' : `${averageTimeToFill}d`, helper: 'Live average', helperColor: '#EF4444' },
        ]}
      />

      <Grid columns={['flex', 'flex']} gap="24px">
        <DashboardSection title="Candidate stage summary">
          {stageData.length ? (
              ...
              <Box justify="center" gap="12px">
                {stageData.map((entry, index) => (
                  ...
                  <Box flex overflow="hidden">
                    <Text size="small" color="#475569" truncate>
                      {entry.name}
                    </Text>
                  </Box>
            ...
            <DashboardEmpty message="No candidate stage data available yet." />
          )}
        </DashboardSection>

        <DashboardSection title="Jobs - assign vendors">
          <Box gap="12px">
            {assignVendorJobs.length ? (
              ...
              <DashboardEmpty message="No jobs available yet." />
            )}
          </Box>
        </DashboardSection>
      </Grid>
      ...
      <Grid columns={['flex', 'flex']} gap="24px">
        <DashboardSection title="Weekly profile submissions">
          {weeklySubmissions.length ? (
            ...
            <DashboardEmpty message="No weekly submission data available yet." />
          )}
        </DashboardSection>

        <DashboardSection title="Vendor activity">
          {vendorActivity.length ? (
            ...
            <DashboardEmpty message="No vendor activity available yet." />
          )}
        </DashboardSection>
      </Grid>
